...
const Profile = () => {
    ...
    const { favorites, favoriteRecipes, loading: loadingFavorites, toggleFavorite } = useFavorites();
    const { categories } = useCategories(2);
    const { activeTab, setActiveTab, currentPage, setCurrentPage, paginatedRecipes, totalPages, filteredRecipes } =
        useProfileRecipes();
            ...
            <div className="mb-6 flex gap-0">
                <button
                    className={`px-2 py-2 rounded font-semibold transition-colors ${
                        activeTab === "saved" ? "bg-red-400 text-white scale-100" : "bg-gray-200 text-gray-600 scale-90"
                    }`}
                                                ...
                                                ? recipe.categories
                                                      .map((catId) => {
                                                          const cat = categories.find(
                                                              (c) =>
                                                                  c.id ===
                                                                  (typeof catId === "object" ? catId.id : catId)
                                                          );
                                                          return cat?.name;
                                                      })
                                                      .filter(Boolean)
